...
/// Defined RV64 `satp.MODE` encodings.
...
impl Mode {
    ...
    /// Decode a mode, returning `None` for reserved encodings.
    ...
    /// Return the mode for `levels`, if defined.
...
impl Satp {
    ...
    /// Return `None` when the mode field contains a reserved encoding.
    ...
    /// Replace the root while preserving mode and ASID.
    ///
    /// # Panics
    ///
