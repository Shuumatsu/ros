...
/// hart's, which the linker provides.
///
...
/// Allocate a kernel stack, at an address from [`kernel_va`].
///
...
/// available. Frames are [`leak`](frame::Frames::leak)ed, since a stack lives as long as
/// its hart.
///
...
///
/// One stack per call, reserved whole, so there is no index to compute wrong and no way
/// for a caller to be handed the same stack twice.
///
/// # Panics
///
...
pub fn alloc(name: &'static str) -> Stack {
    ...
    let stack = Stack { name, pa: frames.leak(), bottom: slot.add(GUARD_SIZE) };
    ALLOCATED
