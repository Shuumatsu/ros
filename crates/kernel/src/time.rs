//! Bounded waits: a free-running counter, and the frequency that gives it units.
//!
//! Both halves in one module because neither is usable alone. The counter ticks at a rate
...
//! conversion to get wrong. Reading it is [`arch::time_counter`]'s; what a tick is worth is
//! this module's.
//!
//! A counter read and a spin, nothing more: no timer interrupt, so this works before there
//! is a trap subsystem to take one.
